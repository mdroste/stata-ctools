...
di "{hline 60}"
di "Testing cqreg Solver Options"
di "{hline 60}"
...
* Test FN solver without preprocessing (nopreprocess=1)
di _newline "Testing Frisch-Newton direct (nopreprocess(1))..."
timer clear 3
timer on 3
cqreg y x1 x2, verbose nopreprocess(1)
timer off 3
quietly timer list 3
di "cqreg (FN):   " r(t3) "s"
matrix b_fn = e(b)
di "  beta: " %9.6f b_fn[1,1] "  " %9.6f b_fn[1,2] "  " %9.6f b_fn[1,3]
...
if `diff1' < 0.1 & `diff2' < 0.1 & `diff3' < 0.1 {
    di "  PASS - coefficients reasonably close"
}
else {
    di "  FAIL - coefficients differ significantly"
}

* Test FN with preprocessing (default)
di _newline "Testing Frisch-Newton with preprocessing (default)..."
timer clear 4
timer on 4
cqreg y x1 x2, verbose nopreprocess(0)
timer off 4
quietly timer list 4
di "cqreg (FN+PP):" r(t4) "s"
matrix b_fnpp = e(b)
di "  beta: " %9.6f b_fnpp[1,1] "  " %9.6f b_fnpp[1,2] "  " %9.6f b_fnpp[1,3]

local diff1 = abs(b_qreg[1,1] - b_fnpp[1,1])
local diff2 = abs(b_qreg[1,2] - b_fnpp[1,2])
local diff3 = abs(b_qreg[1,3] - b_fnpp[1,3])
di "  Diffs: " `diff1' "  " `diff2' "  " `diff3'
