...
static stata_retcode timsort_apply_permutation(stata_data *data)
{
    ...
    if (pool != NULL && nvars >= 2) {
        if (ctools_persistent_pool_submit_batch(pool, timsort_apply_permute_thread,
                                                 args, nvars,
                                                 sizeof(timsort_permute_args_t)) != 0) {
            free(args);
            return STATA_ERR_MEMORY;
        }
...
/*
    ctools_sort_timsort_order_only - Timsort without applying permutation
